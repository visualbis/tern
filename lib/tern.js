...
(function(root, mod) {
  if (typeof exports == "object" && typeof module == "object") // CommonJS
    return mod(exports, require("./infer"), require("./signal"),
               require("acorn/dist/acorn.js"), require("acorn-walk/dist/walk.js"));
  if (typeof define == "function" && define.amd) // AMD
    return define(["exports", "./infer", "./signal", "acorn/dist/acorn.js", "acorn-walk/dist/walk.js"], mod);
  ...
  var addCompletion = exports.addCompletion = function(query, completions, name, aval, depth) {
    ...
    if (aval && typeInfo) {
      ...
      if (query.diplayName)
        maybeSet(rec, "origin", aval.diplayName || type && type.diplayName);
      if (aval.className || type && type.className)
        maybeSet(rec, "className", aval.className || type && type.className);
    }
    if (query.depths) rec.depth = depth || 0;

    if(aval && aval.displayName)rec.displayName = aval.displayName;    
    return rec;
  };
  ...
  function findCompletions(srv, query, file) {
    ...
    function gather(prop, obj, depth, addInfo) {
      ...
      let displayName = obj.props[prop]["displayName"];
      displayName = displayName?displayName:prop;
      let  searchStr= (!query.caseInsensitive ? displayName.toLowerCase() : displayName);
      if (query.filter !== false && word &&
          searchStr.search("(^|\\b)"+word+".*$") < 0) return;
   ...
   if (query.sort !== false) completions.sort(compareCompletions);
    ...
    let updatedCompletionList = [];
    const orderKeys = ["fn", "string", "array", "Object"];
    const group = completions.reduce((r, a) => {
      let type = a.type;
      if (/^fn\(/.test(type)) {
        type = "fn";
      }
    ...
    keys.forEach((key) => {
      ...
      if (list) {
        ...
        updatedCompletionList = updatedCompletionList.concat(list);
      }
    });
    return {start: outputPos(query, file, wordStart),
            ...
            isProperty: !!prop,
            isObjectKey: !!isKey,
            completions: updatedCompletionList};
  }
