...
(function(mod) {
  ...
  function isSimpleAnnotation(spec) {
    ...
    for (var prop in spec)
      if (prop != "!className" && prop != "!type" && prop != "!doc" && prop != "!url" && prop != "!span" && prop != "!data" && prop != "!displayName")
        return false;
    return true;
  }
  ...
  function passOne(base, spec, path) {
    if (!base) {
      ...
      if (tp) {
        ...
        else if (/^THIS/.test(tp)) base = emptyObj(infer.Obj);
    ...
    for (var name in spec) if (hop(spec, name) && name.charCodeAt(0) != 33) {
      ...
      var prop = base.defProp(name,undefined,inner["!displayName"]);
  ...
  function passTwo(base, spec, path) {
    ...
    for (var name in spec) if (hop(spec, name) && name.charCodeAt(0) != 33) {
      var inner = spec[name], known = base.defProp(name,undefined,spec[name]["!displayName"]), innerPath = path ? path + "." + name : name;
      if (typeof inner == "string") {
        ...
        if (inner["!className"]) known.span = inner["!className"];
      }
    }
    return base;
  ...
  function copyInfo(spec, type) {
    ...
    if (spec["!displayName"]) type.displayName = spec["!displayName"];
    if (spec["!className"]) type.className = spec["!className"];
  }
